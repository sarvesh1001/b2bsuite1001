...
export default function App() {
  ...
  // 5. Proactive refresh timer (every 4.5 minutes) – now with 401 handling
  useEffect(() => {
    const startTimer = () => {
      ...
      refreshTimerRef.current = setInterval(() => {
        if (useAuthStore.getState().isAuthenticated) {
          ...
          doRefresh().catch((err) => {
            ...
            // 🔥 ADDED: handle 401 on timer refresh
  ...
  }, [isAuthenticated, doRefresh, clearSession]);

  // 6. Proactive Refresh on Launch – now with 401 handling
  useEffect(() => {
    async function refreshOnLaunch() {
      ...
      try {
        ...
        // 🔥 ADDED: handle 401 on launch refresh
  ...
  }, [isReady, fontsLoaded, doRefresh, logout, clearSession]);
