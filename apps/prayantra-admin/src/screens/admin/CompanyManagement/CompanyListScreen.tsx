// apps/mobile/src/screens/admin/CompanyManagement/CompanyListScreen.tsx
...
export default function CompanyListScreen() {
  ...
  // Client-side plan filter applied on top of the fetched list
  const visibleCompanies =
    planFilter === 'all'
      ? companies
      : companies.filter((c) => c.subscription_plan_code === planFilter);

  // Distinct plans available in current result set
  const availablePlans = Array.from(
    new Set(companies.map((c) => c.subscription_plan_code).filter(Boolean) as string[])
  );
          ...
          <View style={styles.cardHeader}>
            ...
            <View style={styles.headerBadges}>
              ...
              {item.subscription_plan_code ? (
                <View style={styles.planChipCustom}>
                  <Text style={styles.planChipText}>{item.subscription_plan_code}</Text>
                </View>
              ) : null}
            </View>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      ...
      {/* Plan filter row (only shown if plans exist in current list) */}
        ...
        <View style={styles.filterRow}>
          ...
          >
            <Chip
              ...
              style={[styles.filterChip, planFilter === 'all' && styles.activePlanChip]}
              textStyle={[
                styles.filterChipText,
                planFilter === 'all' && styles.activeChipText,
              ]}
            >
              All Plans
            </Chip>
            {availablePlans.map((p) => (
              <Chip
                key={p}
                selected={planFilter === p}
                onPress={() => setPlanFilter(p)}
                style={[styles.filterChip, planFilter === p && styles.activePlanChip]}
                textStyle={[
                  styles.filterChipText,
                  planFilter === p && styles.activeChipText,
                ]}
              >
                {p}
              </Chip>
...
const styles = StyleSheet.create({
  ...
  activePlanChip: { backgroundColor: '#7B2FBE' },
  ...
  tierText: { fontSize: 11, fontWeight: '500', color: '#333' },
  planChipCustom: {
  ...
  planChipText: { fontSize: 11, fontWeight: '600', color: '#00695C' },
  ownerText: { color: '#666', marginTop: 4, fontSize: 13 },
  metaRow: { flexDirection: 'row', marginTop: 6, flexWrap: 'wrap' },
  metaItem: { flexDirection: 'row', alignItems: 'center', marginRight: 16 },
  metaText: { color: '#888', fontSize: 12, marginLeft: 4 },
  emptyContainer: {
    flex: 1,
