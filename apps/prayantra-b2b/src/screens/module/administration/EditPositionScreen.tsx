...
type EditPositionRouteProp = RouteProp<RootStackParamList, 'EditPosition'>;
type NavigationProp = StackNavigationProp<RootStackParamList, 'EditPosition'>;

// Schema – all fields optional, but we'll ensure data exists
...
export default function EditPositionScreen() {
  ...
  const route = useRoute<EditPositionRouteProp>();
    ...
    const fetchData = async () => {
      ...
      try {
        const [positionRes, deptRes, wcRes] = await Promise.all([
          getPosition(companyId, deviceId!, positionId, accessToken),
        ...
        if (!position) {
          Alert.alert('Not Found', 'Position not found');
          navigation.goBack();
          return;
        }
        ...
        Alert.alert('Error', error.message || 'Failed to load position');
        navigation.goBack();
      } finally {
        setLoading(false);
  ...
  // Submit – convert null to undefined for API
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      await updatePosition(companyId, deviceId!, positionId, payload, accessToken);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      {/* Modal – separate FlatLists for each type */}
      <Modal
