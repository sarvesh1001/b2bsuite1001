...
import { assignStructureToEmployee } from '@b2b/api-client';
import {
  BACKGROUND_COLOR,
  CARD_BACKGROUND,
  PRIMARY_COLOR,
...
type NavigationProps = StackNavigationProp<RootStackParamList, 'AssignStructure'>;
type RouteProps = RouteProp<RootStackParamList, 'AssignStructure'>;

export default function AssignStructureToEmployee() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await assignStructureToEmployee(companyId, deviceId, accessToken, {
        user_id: userId.trim(),
      ...
      Alert.alert('Success', 'Structure assigned to employee');
      navigation.goBack();
    } catch (error: any) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Assign Structure</Text>
        <View style={{ width: 24 }} />
      </View>
