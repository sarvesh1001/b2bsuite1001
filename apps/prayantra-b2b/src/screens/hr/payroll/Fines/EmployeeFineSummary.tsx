...
import { getFineSummaryByEmployee } from '@b2b/api-client';
import {
  BACKGROUND_COLOR,
  CARD_BACKGROUND,
  PRIMARY_COLOR,
...
type NavigationProps = StackNavigationProp<RootStackParamList, 'EmployeeFineSummary'>;

export default function EmployeeFineSummary() {
  ...
  const fetchSummary = async () => {
    ...
    try {
      ...
      const res = await getFineSummaryByEmployee(companyId, userId.trim(), deviceId, accessToken, params);
      setSummary(res.data);
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Employee Fine Summary</Text>
        <View style={{ width: 24 }} />
      </View>
        ...
        >
          {summary ? (
            <>
              ...
              {summary.fines && summary.fines.length > 0 && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Recent Fines</Text>
                  {summary.fines.slice(0, 5).map((fine: any) => (
                    <View key={fine.id} style={styles.fineItem}>
                      <Text style={styles.fineAmount}>₹{fine.fine_amount}</Text>
                      <Text style={styles.fineReason}>{fine.reason}</Text>
                      <Text style={styles.fineDate}>{new Date(fine.fine_date).toLocaleDateString()}</Text>
                    </View>
                  ))}
                </View>
              )}
            ...
            <View style={styles.empty}>
              <Text style={styles.emptyText}>Enter a User ID and search to see summary</Text>
            </View>
          )}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  fineAmount: { fontSize: 14, fontWeight: '500', color: TEXT_PRIMARY },
  fineReason: { flex: 1, fontSize: 13, color: TEXT_SECONDARY, marginHorizontal: 8 },
  fineDate: { fontSize: 12, color: TEXT_SECONDARY },
